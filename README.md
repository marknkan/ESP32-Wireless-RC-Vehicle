...
Wireless RC vehicle using two ESP32 boards, ESP-NOW communication and a TB6612FNG motor driver.
...
See docs/test_results.md for the acceptance-test table
and limitations.
...
Connect the vehicle ESP32 and upload
firmware/vehicle_receiver/VehicleReceiver.ino.

If replacing either ESP32, run the MAC-address utility and update
VEHICLE_MAC in the controller sketch.

Connect the controller ESP32 and upload
firmware/controller_sender/ControllerSender.ino.

Confirm both devices use ESP-NOW channel 1.

Raise the vehicle wheels for the first test, then power the controller.

Verify Stop and communication-loss behaviour before floor operation.
